...
<!-- QH: Trying to think of a way to make this figure work with the current figure numbering system we have in bookdown. Adding in a number that starts before 1.1 for now -->
## Methods

![Figure 1.0. Schematic figure showing both the intial measurements that were used to generate our landmarks and the location of our landmarks on a hypothetical flake. A, B, C, D, and E are the top horizontal measurement, width at a quarter of the maximum length, maximum dimension length, width at a quarter of the maximum length, and width at three quarter of the maximum length respectively. Points 1 through 9 are all of the landmarks generated from measurements A through E.  ](./test_graphic.png)
...
```{r, fig.cap="PCA biplot for flake shape and dorsal cortex class."}
...
```{r, fig.cap="PCA biplot for flake shape and excavation unit."}
# Excavation unit PCA
# ggplot2-based plot of the PCA results using a factoextra function
...
```{r, fig.cap="PCA biplot for flake shape and mass cluster groups"}
...
<!-- Below are my attempts to create a nice looking plot_grid of the three biplots. I was not able to get it to display properly, so I am just printing off each biplot individually all in a row as a temporary measure.  -->
```{r}
library(cowplot)
...
# plot_grid(
#  cortex_biplot, NULL, NULL,  excavation_unit_biplot,  mass_biplot, NULL,
#  ncol = 2,
#  labels = c("A", NULL, NULL,"B","C",NULL),
#  label_size = 12,
#  align = "v",
#  rel_heights = c(1.1,
#                  1,
...
#cortex_biplot + theme(plot.margin = unit(c(3,3,3,3), "lines"))
#excavation_unit_biplot
#mass_biplot
...
long_labels <- 
  tribble(       ~long_labels,             ~short_labels, 
                 "X Value: Top Left",         "1.X",
                 "Y Value: Top Left",         "2.X",
...
# Discussion
 ...
 <!-- QH: As I write this I notice a major issue with the axes labels on the contribution plots. Instead of substituting the original labels on a one to one basis with our English labels, our plots are just printing out our list of English labels in the order they are listed in our code. This has to be fixed. I then need to redo my description of the plots, since it is all wrong and based off of the eroneous labels. -->

Looking at \@ref(fig:contribution-plots), the landmarks which contributed the most to each principal component were all located on the left side of our artefacts. This means that linking our initial flake measurements (maximum length, top width etc.) to either of the first two principal components is nearly impossible, as every one of our initial flake measurements (with the)

one can see that there is no consistent pattern in which certain initial flake measurements  align closely with either the first or second principal components. Interestingly, the landmarks which contributed the most to each principal component were all located on the left side of our artefacts..
