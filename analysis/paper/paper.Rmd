...
# This is a temporary fix just to get the labels looking nice for now; there is most certainly a cleaner way to do this
# My fix did not work. I will have to create a temp table showing the values for now


# PC1_contrib[["plot_env"]][["xx"]] <- as.factor(c("X Value: Top Left",
#                                              "Y Value: Top Left",
...
#PC1_contrib[["plot_env"]][["data"]][["name"]][PC1_contrib[["plot_env"]][["data"]][["name"]] == '5.X'] <- "X Value: Distal Tip"



#PC1_contrib[["plot_env"]][["data_sum"]][["name"]] <- as.factor(c(1:18))

output_meanings <-tibble(data.frame(c("X Value: Top Left",
         "Y Value: Top Left",
...
output_meanings <- setNames(output_meanings, c("Landmark Value", "Computer Output"))
...
```
  ...
  <!-- Could not figure out how to change the labels on the x axis for the two bar charts, so I included a small table with the human-meaningful values for now. This must be fixed later-->
  
  Figure \@ref(fig:contribution_plots) shows the relative contribution of each of the original 18 variables put through our PCA. Looking at the supplementary table, we can see that the first principal component is primarily composed of the x-value for the distal tip landmark, with smaller contributions from the y-values from the two upper-most and mid-upper points. The second primary dimension is composed largely of the y-values for bottom-most and upper-most points. 
